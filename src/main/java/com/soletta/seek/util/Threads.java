...
public class Threads {
    ...
    public static ExecutorService newFixedThreadPool(String name, int nThreads, long timeout, TimeUnit units) {
        ThreadPoolExecutor ret = new ThreadPoolExecutor(nThreads, nThreads, timeout, units, new LinkedBlockingQueue<Runnable>(),
    ...
    /**
     * Create a ThreadFactory inside a new thread group nested inside the current thread group. ThreadFactories are used
     * by java.util.concurrent to create threads as necessary.
     * 
     * @param name
     * @return GroupThreadFactory
     */
    ...
    /**
     * Create a ThreadFactory inside a new thread group nested inside the current thread group; runtime exceptions are
     * routed to the exceptionLog you provide. ThreadFactories are used by java.util.concurrent to create threads as
     * necessary.
     * 
     * @param name
     * @param exceptionLog
    ...
    /**
     * Create a monitor for the calling thread; a monitor is a secondary thread that will periodically invoke the
     * specified callback, on the provided delay schedule. delay[0] is the first delay, followed by delay[1], and so
     * forth. The last provided delay is repeated.
     * <p>
     * When the calling thread finishes its activity, it should cancel the future that is returned. This will cancel the
     * monitor, and no further callbacks will occur.
     * 
     * @param callback
     * @param delayMillis
     *            The callback delay, in milliseconds.
     * @return Future<Void>
     */
    ...
    /**
     * Create a monitor for a specified thread; a monitor is a secondary thread that will periodically invoke the
     * specified callback, on the provided delay schedule. delay[0] is the first delay, followed by delay[1], and so
     * forth. The last provided delay is repeated.
     * <p>
     * When the calling thread finishes its activity, it should cancel the future that is returned. This will cancel the
    ...
    /**
     * Defines the callback for monitors. The thread being monitored is passed to the callback, which can interrupt the
    ...
    public interface Monitor {
        /**
         * Tells the monitored thread how long it has been alive, and how many times it has been called back. The
         * callback can interrupt the thread if it wants to.
         * 
         * @param t
